...
namespace LobbyServer.Helper
{
    ...
    public sealed class ConfigHelper
    {
        ...
        private int _port = 0;
        private int _opPort = 0; // 존서버들이 붙는 포트
        ...
        public int OpPort => _opPort; // 존서버들이 붙는 포트
        ...
        public string MongDbConnectString => _mongodbConnectString;
        ...
        // SWaggerUI 표시
        public bool EnableSwaggerUi => _enableSwaggerUi;
...
#pragma warning disable CS8602, CS8604 // null 가능 참조에 대한 역참조입니다.
            ...
            _enableSwaggerUi = _jsonObj["swagger"]["ui"].Value<bool>();
...
#pragma warning restore CS8602, CS8604 // null 가능 참조에 대한 역참조입니다.
            ...
            ActorPaths.System = "LobbyServer";

            if (false == LoadWorldInfo())
                return false;

            if (false == LoadAbusiveNicknames())
            {
                _logger.Error($"failed to load ProhibitedNicknames");
                return false;
            }

            // Firebase를 사용하기 위한 처리
            if (false == string.IsNullOrEmpty(directoryPath))
            {
                if (false == LoadFirebase(directoryPath))
                    return false;
            }

            // 애플 키 관련
            AppleJwtTokenCache.Instance.LoadApplePrivateKey();

            _logger.Warn($"Load Config Success ");
...
#pragma warning restore CS8602, CS8604 // null 가능 참조에 대한 역참조입니다.
        ...
        public bool LoadFirebase(string directoryPath)
        {
            try
            {
                ...
                FirebaseAppSingleton.ServerAccountKeyPath = pathToServiceAccountKeyFile;
                var instance = FirebaseAppSingleton.Instance;

                _logger.InfoEx(() =>"LoadFirebase Success");

                return true;
            }
            catch (Exception ex)
            {
                _logger.Error(() => "failed to LoadFirebase", ex);
                return false;
            }
        }
        ...
        private bool LoadWorldInfo()
        {
            ...
            if (serverInfo.server_id == 0)
            {
                _logger.Error($"failed LoadWorldInfo() server_id:{serverInfo.server_id}");
                return false;
            }
            ...
            _opPort = serverInfo.op_port;
            if (_opPort == 0)
            {
                _logger.Error($"not found op port. serverId:{serverId}");
                return false;
            }
        ...
        /// </summary>
        /// <returns></returns>
        private bool LoadAbusiveNicknames()
        {
            var fullPath = Assembly.GetExecutingAssembly().Location;
            var directoryPath = Path.GetDirectoryName(fullPath);

            //string filePath = $@"{directoryPath}/AbusiveName.slk";
            //var rtn = AbusiveNamesHelper.Instance.LoadSlk(filePath);

            string filePath = $@"{directoryPath}/AbusiveName.txt";
            var rtn = AbusiveNamesHelper.Instance.LoadText(filePath);
